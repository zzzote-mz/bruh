...
from info import LOG_CHANNEL, ADMINS, AUTH_CHANNEL, AUTH_USERS, CUSTOM_FILE_CAPTION, AUTH_GROUPS, P_TTI_SHOW_OFF, IMDB, SINGLE_BUTTON, SPELL_CHECK_REPLY, IMDB_TEMPLATE
...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("itih tur anilo", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
    ...
    if not search:
        await query.answer("i ti thei tawhlo ang, ka message hi a hlui tawh a, a thar in ti leh mai rawh",show_alert=True)
        return
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        if not files_:
            return await query.answer('File hmuh anilo.')
        files = files_[0]
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("ila join lo",show_alert=True)
            return
        ident, file_id = query.data.split("#")
        files_ = await get_file_details(file_id)
        if not files_:
            return await query.answer('File a awm rihlo')
        files = files_[0]
        title = files.file_name
        size=get_size(files.file_size)
        f_caption=files.caption
        ...
        buttons = [[
            InlineKeyboardButton('Tutorial', url='https://youtu.be/5dpRZY_CsJY'),
            ],[
...
async def advantage_spell_chok(client, msg):
    ...
    if not movielist:
        await client.send_message(text="i duh hi ka zawng hmulo tlat mai, ka database ah a awmlo emaw i zawn dan leh ka database a a awmdan a in an loh vang pawh ani maithei ani, i duh chuan a hnuaia **Files** tih button khu hmet la, kha mi channel ah khan i zawn hmuh loh kha zawng rawh, ala awmlo fo anih chuan request dan tur ang khan i request thei ang.", reply_to_message_id=msg.message_id, reply_markup=InlineKeyboardMarkup(btt))
        return
